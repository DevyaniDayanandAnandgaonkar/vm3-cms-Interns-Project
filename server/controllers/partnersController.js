...
exports.createPartner = async (req, res) => {
  ...
  try {
    ...
    if (!partner_name || !email) {
      return res.status(400).json({ message: 'partner_name and email are required' });
    }
    ...
    const params = [partner_name, email, hashedPassword, phone || null, address || null, status || 'Active'];

    const [result] = await db.query(q, params);
    const [newPartner] = await db.query('SELECT * FROM associate_partners WHERE partner_id = ?', [result.insertId]);
...
exports.updatePartner = async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.query('SELECT * FROM associate_partners WHERE partner_id = ?', [partnerId]);
    if (rows.length === 0) return res.status(404).json({ message: 'Partner not found' });

    const updates = [];
    const params = [];

    if (partner_name !== undefined) { updates.push('partner_name = ?'); params.push(partner_name); }
    ...
    const q = `UPDATE associate_partners SET ${updates.join(', ')} WHERE partner_id = ?`;
    params.push(partnerId);

    await db.query(q, params);
    
    const [updatedPartner] = await db.query('SELECT * FROM associate_partners WHERE partner_id = ?', [partnerId]);
